...
export default function Users() {
    ...
    setConfirmDialog({
      ...
      action: async () => {
        try {
          ...
          await Promise.all(Array.from(selection.selectedIds).map((id) => deleteUser(id).unwrap()));
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV(dataToExport, `users-${new Date().toISOString().split('T')[0]}.csv`, [
      { key: 'name', label: 'Name' },
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(dataToExport, `users-${new Date().toISOString().split('T')[0]}`, 'Users', [
      { key: 'name', label: 'Name' },
      { key: 'email', label: 'Email' },
      { key: 'phone', label: 'Phone' },
      { key: 'status', label: 'Status', format: (val) => (val === 1 ? 'Active' : 'Suspended') },
